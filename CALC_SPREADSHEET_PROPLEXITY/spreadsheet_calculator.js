...
function savePreview() {
  const blob = new Blob([document.getElementById('clipboardPreview').value], { type: 'text/csv' });
...
function downloadCsvWithState(state) {
    ...
    .then(blob => {
        ...
        link.download = `spreadsheet_export_${Date.now()}.csv`;
...
document.getElementById('exportCsvBtn').addEventListener('click', () => {
    // Optionally append query params for filtering, pagination, etc.
    const url = 'export_csv.php';
    window.location.href = url;
});

// Usage: pass your current grid data
// downloadCsvWithState(currentGridData);
/* spreadsheet_calculator.css */
function downloadCsvWithState(state) {
    fetch('export_csv.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
